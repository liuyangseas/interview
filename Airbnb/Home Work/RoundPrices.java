...
public class Float_Ceil_or_Floor {
	static class NumWithDiff {
        ...
        double diffWithCeil;
        int index;
        public NumWithDiff(int n, double c, int index) {
                this.num = n;
                this.diffWithCeil = c;
                this.index = index;
        }
    ...
    public static int[] getNearlyArrayWithSameSum(double[] arr) {
            ...
            for (int i = 0; i < arr.length; i++) {
                    ...
                    floorSum += floor;
                    sum += arr[i];
                    arrWithDiff[i] = new NumWithDiff(ceil, ceil - arr[i], i); // 把ceil都放在数组里面进行比较
            }
            ...
            Arrays.sort(arrWithDiff, (a, b) -> (int) (a.diffWithCeil- b.diffWithCeil));
            int[] res = new int[arr.length];
            /*
            int i = 0;
            for (; i < diff; i++) {
                res[i] = arrWithDiff[i].num; // 这些放ceiling
            }            
            for (; i < arr.length; i++) {
            ...
            */
            ...
            int i = 0;
            for (; i < diff; i++) {
                res[arrWithDiff[i].index] = arrWithDiff[i].num; // 这些放ceiling
            }
            for (; i < arr.length; i++) {
                res[arrWithDiff[i].index] = arrWithDiff[i].num - 1; // 剩下的只放floor  
            }
